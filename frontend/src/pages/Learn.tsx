...
export default function Learn() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-[var(--space-section)]">
      ...
      <div className="grid grid-cols-1 gap-[var(--space-card-grid-lg)] xl:grid-cols-2 2xl:grid-cols-12">
        {/* Repeat More */}
        <div className="rounded-2xl border border-[var(--color-aria-green-dim)] bg-[var(--color-aria-surface)] p-6 2xl:col-span-6">
          ...
          {repeatMore.length ? (
            <ul className="space-y-[var(--space-block)]">
              {repeatMore.map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm">
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-aria-green)]" />
        ...
        {/* Reduce or Fix */}
        <div className="rounded-2xl border border-red-500/15 bg-[var(--color-aria-surface)] p-6 2xl:col-span-6">
          <div className="mb-[var(--space-default)] flex items-center gap-[var(--space-tight)]">
          ...
          {reduceOrFix.length ? (
            <ul className="space-y-[var(--space-block)]">
              {reduceOrFix.map((item) => (
                <li key={item} className="flex items-start gap-2 text-sm">
                  <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--color-aria-red)]" />
                  <span className="text-[var(--color-aria-muted)]">{item}</span>
                </li>
              ))}
            </ul>
      ...
      <div className="rounded-2xl border border-[var(--color-aria-border)] bg-[var(--color-aria-surface)] p-6">
        ...
        {titlePatterns.length ? (
          <div className="flex flex-wrap gap-2">
            {titlePatterns.map((p) => (
              <div key={String(p.pattern)} className="flex items-center gap-2 rounded-full border border-[var(--color-aria-border)] bg-[var(--color-aria-surface-3)] px-3 py-2">
      ...
      {!pm && (
        <div className="flex items-center gap-3 rounded-2xl border border-[var(--color-aria-border)] bg-[var(--color-aria-surface)] p-6">
          <Brain className="h-8 w-8 shrink-0 text-[var(--color-aria-faint)]" />
          <div>
            <p className="text-sm font-semibold text-[var(--color-aria-ink)]">No memory snapshot yet</p>
            <p className="mt-2 text-xs text-[var(--color-aria-muted)]">Connect YouTube and sync data to let A.R.I.A. start building your creator memory.</p>
          </div>
        </div>
      )}
    </motion.div>
